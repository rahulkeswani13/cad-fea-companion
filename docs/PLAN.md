...
| **F31** | **Console clarity & demo refinement (ADR-017, 4 PRs)**: honest origin labels from wire evidence (Analytical estimate / Saved reference result / Fallback result / Live simulation; fallback overrides live-sounding labels); execution-status stamps (Completed/Failed) replacing PASS/FAIL verdicts; numeric honesty (missing → em dash, zeros preserved); friendly tool names; Sources visible + Technical details; task-oriented prompt library + Try-validation-errors group; 3 guided customer journeys in frontend config referencing canonical prompts; every entry point fills the composer (only Send executes); fresh-on-load session semantics with Runs-in-this-session (run_id dedup, sub-runs, unrecorded markers), obsolete-response guard, saved-workspace labeling, Recent saved runs disclosure; numbered prefixes and feature ids removed from visible UI — **done 2026-09-06 (ADR-017; PRs `codex/console-honesty-pass`, `codex/console-demo-flow`, `codex/console-session-semantics`, `codex/console-cleanup`)** | 2 | R4 | F29 | Browser PART 4/5 suites + legacy 45 stay green; eval `tool_reject_unknown_material`; eval delta none for presentation-only phases (documented); committed build output rebuilt each PR |
